...
void genotype::read_bed_mailman (std::istream& ifs,string filename,bool allow_missing)  {
...
void genotype::read_bed (std::istream& stream,string filename, bool allow_missing, bool mailman_mode )  {
	/*if(mailman_mode){
		if (allow_missing)
	...
	if(mailman_mode){
		read_bed_mailman( stream,filename,allow_missing);
	}
	else{
		read_bed_naive(filename,allow_missing);
	}
...
void genotype::read_plink(std::istream& ifs,std::string filenameprefix, bool allow_missing,bool mailman_mode)  { 
	...
	read_bed ( ifs,f3.str(), allow_missing,mailman_mode);
}
